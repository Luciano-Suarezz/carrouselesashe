...
import { Send, Loader2, Save, PlusCircle, Bot, User as UserIcon, Check, Upload, X, Copy, BookOpen, Trash2 } from 'lucide-react';
...
interface GenChatProps {
  ...
  onSavePromptToLibrary: (name: string, prompt: string) => Promise<void>;
  onDeletePromptFromLibrary: (id: string) => Promise<void>;
  onImportPrompts: (background: string | undefined, slides: string[]) => void;
  apiKey: string;
}

export const GenChat: React.FC<GenChatProps> = ({ 
  initialSystemPrompt, 
  savedPrompts,
  onSaveSystemPrompt, 
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      await onSavePromptToLibrary(newPromptName, systemPrompt);
      setNewPromptName('');
    } catch (e) {
  ...
  const handleLoadFromLibrary = (prompt: string) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 text-gray-200 relative">
      ...
      {isLibraryModalOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl flex flex-col overflow-hidden shadow-2xl max-h-[80vh]">
            <div className="flex items-center justify-between p-4 border-b border-gray-800 bg-gray-950">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-primary-500" />
                System Prompts Library
              </h2>
              <button onClick={() => setIsLibraryModalOpen(false)} className="p-1 text-gray-400 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            
            <div className="p-4 border-b border-gray-800 bg-gray-900/50">
              <h3 className="text-sm font-bold text-gray-400 mb-2">Save Current Prompt</h3>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={newPromptName}
                  onChange={(e) => setNewPromptName(e.target.value)}
                  placeholder="e.g., Aggressive Marketing Persona"
                  className="flex-1 bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 text-sm text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
                />
                <button
                  onClick={handleSaveToLibrary}
                  disabled={!newPromptName.trim() || !systemPrompt.trim() || isSavingToLibrary}
                ...
                >
                  {isSavingToLibrary ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                  Save
                </button>
              </div>
            </div>
            ...
            <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
              <h3 className="text-sm font-bold text-gray-400 mb-3">Saved Prompts</h3>
              {savedPrompts.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-8">No saved prompts yet.</p>
              ) : (
                <div className="space-y-3">
                  {savedPrompts.map((p) => (
                    <div key={p.id} className="bg-gray-950 border border-gray-800 rounded-xl p-4 flex flex-col gap-2 group">
                      <div className="flex items-center justify-between">
                        <h4 className="font-bold text-primary-400">{p.name}</h4>
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => handleLoadFromLibrary(p.prompt)}
                            className="text-xs font-bold bg-primary-600 hover:bg-primary-500 text-white px-3 py-1.5 rounded transition-colors"
                          >
                            Use
                          </button>
                          <button
                            onClick={() => onDeletePromptFromLibrary(p.id)}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-xs font-bold text-gray-400 hover:text-white flex items-center gap-1 transition-colors bg-gray-800 hover:bg-gray-700 px-2 py-1 rounded border border-gray-700"
              >
